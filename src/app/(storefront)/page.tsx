...
export default async function HomePage() {
  ...
  return <main id="main-content" className="storefront sf-home"><HomeReveal />
    {sections.hero.isActive ? <section className="sf-hero"><div className="sf-hero-copy"><p className="sf-overline">Collection Ben Ami <span>Mode · Sneakers · Sélection</span></p><h1>{sections.hero.title}</h1><p>{sections.hero.subtitle}</p><div>{sections.hero.ctaLabel && sections.hero.ctaLink ? <Link className="sf-button dark" href={sections.hero.ctaLink}>{sections.hero.ctaLabel} <span aria-hidden="true">↗</span></Link> : null}<Link className="sf-text-link" href="/#categories">Explorer les catégories <span aria-hidden="true">↓</span></Link></div><small>Des pièces actuelles, choisies avec exigence</small></div><div className="sf-hero-visual">{heroProduct?.images[0] ? <Link href={`/produits/${heroProduct.slug}`} aria-label={`Découvrir ${heroProduct.name}`}><Image src={heroProduct.images[0].url} alt={heroProduct.images[0].alt || heroProduct.name} fill priority sizes="(max-width: 699px) 100vw, 55vw" /><span><small>Pièce en vedette</small>{heroProduct.name} <b aria-hidden="true">Découvrir →</b></span></Link> : <div className="sf-hero-placeholder"><span>BEN AMI</span><p>Mode · Chaussures · Accessoires</p></div>}</div></section> : null}
